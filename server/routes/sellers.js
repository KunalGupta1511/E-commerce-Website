...
router
    ...
    .get("/placed", auth, isSeller, async (req, res) => {
        try {
            const orders = await Orders.find({ seller: req.user._id }).populate("products.productId");
    ...
    //Viewing all products uploaded
    .get("/view/products", auth, isSeller, async (req, res) => {
        const sellerId = req.user._id;

        try {
            const productsUploaded = await Products.find({ postedBy: sellerId });

            return res.status(200).json({ productsUploaded });
        } catch (error) {
    ...
    //Seller dashboard
    .get("/seller/dashboard", auth, isSeller, async (req, res) => {
        try {
            const orders = await Orders.find({ seller: req.user._id }).populate("products.productId");
            const productsUploaded = await Products.find({ postedBy: req.user._id });

            const noOfOrders = orders.length;
            const noOfProducts = productsUploaded.length;
            ...
            return res.status(200).json({ noOfOrders, noOfProducts, count });
        } catch (err) {
            return res.status(500).json({ error: err.message })
        }
    })
